<?php
	...
	class newpost
	{
		...
		public function set_message($message)
		{
			...
			$this->posttags	= array();
			if( preg_match_all('/\#([а-яa-z0-9\-_]{1,50})/iu', $message, $matches, PREG_PATTERN_ORDER) ) {
		...
		public function save()
		{
			...
			if( ! $this->id )
			{
				...
				$db2->query('UPDATE post_api SET total_posts=total_posts+1 WHERE id="'.$db_api_id.'" LIMIT 1');
			}
			else
			{
				$id	= $this->id;
