<?php
	...
	$D->hdr_custom_logo	= empty($C->HDR_CUSTOM_LOGO) ? ('design/logo.gif') : ('attachments/'.$this->network->id.'/'.$C->HDR_CUSTOM_LOGO);
	$D->hdr_show_favicon	= $C->HDR_SHOW_FAVICON;
	$D->hdr_custom_favicon	= empty($C->HDR_CUSTOM_FAVICON) ? ('design/favicon.ico') : ('attachments/'.$this->network->id.'/'.$C->HDR_CUSTOM_FAVICON);
				...
				elseif( empty($C->HDR_CUSTOM_LOGO) && !$f ) {
					...
					elseif( $w < $C->LOGO_HEIGHT ) {
						...
						if( ! file_exists($path.$fn) ) {
							...
							if( !empty($C->HDR_CUSTOM_LOGO) ) {
								rm( $path.$C->HDR_CUSTOM_LOGO );
							}
							$db2->query('REPLACE INTO settings SET word="HDR_SHOW_LOGO", value="2" ');
