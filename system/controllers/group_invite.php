<?php
	...
	$cannot_invite	= $this->network->get_group_members($g->id);
	$cannot_invite[$this->user->id]	= 1;
	
	$tmp	= array();
	$r	= $db2->query('SELECT id FROM users WHERE active=1 ORDER BY username ASC');
	while($o = $db2->fetch_object($r)) {
		if( isset($cannot_invite[$o->id]) ) {
			continue;
		}
		$o	= $this->network->get_user_by_id($o->id);
		if( ! $o ) {
			continue;
		}
		$tmp[$o->id]	= $o;
