	...
	switch($tab)
	{
		case 'all':
			if($filter == 'all') {
				...
				$q1	= 'SELECT COUNT(b.post_id) FROM post_userbox b, posts_attachments a WHERE b.post_id=a.post_id AND a.type="'.$at_tmp[$filter].'" AND b.user_id="'.$this->user->id.'" ';
		...
		case 'feeds':
			if($filter == 'all') {
				...
				$q1	= 'SELECT COUNT(b.post_id) FROM post_userbox_feeds b, posts_attachments a WHERE b.post_id=a.post_id AND a.type="'.$at_tmp[$filter].'" AND b.user_id="'.$this->user->id.'" ';
		...
		case '@me':
			if($filter == 'all') {
				//$q1	= 'SELECT COUNT(p.id) FROM posts p, posts_mentioned m WHERE p.id=m.post_id '.$not_in_groups.' AND m.user_id="'.$this->user->id.'"';
				//$q2	= 'SELECT p.*, "public" AS `type` FROM posts p, posts_mentioned m WHERE p.id=m.post_id '.$not_in_groups.' AND m.user_id="'.$this->user->id.'" ORDER BY p.id DESC ';
				$q1	= 'SELECT COUNT(DISTINCT p.id) FROM posts p WHERE p.id IN(SELECT post_id FROM posts_mentioned WHERE user_id="'.$this->user->id.'" UNION SELECT post_id FROM posts_comments WHERE id IN(SELECT comment_id FROM posts_comments_mentioned WHERE user_id="'.$this->user->id.'")) '.$not_in_groups;
				$q2	= 'SELECT DISTINCT p.*, "public" AS `type` FROM posts p WHERE p.id IN(SELECT post_id FROM posts_mentioned WHERE user_id="'.$this->user->id.'" UNION SELECT post_id FROM posts_comments WHERE id IN(SELECT comment_id FROM posts_comments_mentioned WHERE user_id="'.$this->user->id.'")) '.$not_in_groups.' ORDER BY p.id DESC ';
			}
			else {
				//$q1	= 'SELECT COUNT(p.id) FROM posts p, posts_mentioned m, posts_attachments a WHERE p.id=a.post_id '.$not_in_groups.' AND p.id=m.post_id AND m.user_id="'.$this->user->id.'" AND a.type="'.$at_tmp[$filter].'"';
				//$q2	= 'SELECT p.*, "public" AS `type` FROM posts p, posts_mentioned m, posts_attachments a WHERE p.id=a.post_id '.$not_in_groups.' AND p.id=m.post_id AND m.user_id="'.$this->user->id.'" AND a.type="'.$at_tmp[$filter].'" ORDER BY p.id DESC ';
				$q1	= 'SELECT COUNT(DISTINCT p.id) FROM posts p, posts_attachments a WHERE p.id=a.post_id AND p.id IN(SELECT post_id FROM posts_mentioned WHERE user_id="'.$this->user->id.'" UNION SELECT post_id FROM posts_comments WHERE id IN(SELECT comment_id FROM posts_comments_mentioned WHERE user_id="'.$this->user->id.'")) '.$not_in_groups.' AND a.type="'.$at_tmp[$filter].'"';
				$q2	= 'SELECT DISTINCT p.*, "public" AS `type` FROM posts p, posts_attachments a WHERE p.id=a.post_id AND p.id IN(SELECT post_id FROM posts_mentioned WHERE user_id="'.$this->user->id.'" UNION SELECT post_id FROM posts_comments WHERE id IN(SELECT comment_id FROM posts_comments_mentioned WHERE user_id="'.$this->user->id.'")) '.$not_in_groups.' AND a.type="'.$at_tmp[$filter].'" ORDER BY p.id DESC ';
			}
			break;
		...
		case 'private':
			...
			elseif($privtab == 'sent') {
				if($filter == 'all') {
					...
					$q1	= 'SELECT COUNT(p.id) FROM posts_pr p, posts_attachments_pr a WHERE p.id=a.post_id AND p.user_id="'.$this->user->id.'" AND a.type="'.$at_tmp[$filter].'"';
			...
			elseif($privtab == 'usr' && $privusr) {
				if($filter == 'all') {
					...
					$q1	= 'SELECT COUNT(p.id) FROM posts_pr p, posts_attachments_pr a WHERE p.id=a.post_id AND ((p.user_id="'.$this->user->id.'" AND p.to_user="'.$privusr->id.'") OR (p.user_id="'.$privusr->id.'" AND p.to_user="'.$this->user->id.'" AND p.is_recp_del=0)) AND a.type="'.$at_tmp[$filter].'"';
