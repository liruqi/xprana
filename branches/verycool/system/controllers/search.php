<?php
	...
	if( isset($_POST['lookin']) && $_POST['lookin']=='posts' && isset($_POST['lookfor'], $_POST['puser'], $_POST['pgroup'], $_POST['pdate1'], $_POST['pdate2']) ) {
		...
		$pgroup	= preg_replace('/[^a-zа-я0-9\-\.\s]/iu', '', $_POST['pgroup']);
	...
	if( isset($_POST['lookin'], $_POST['lookfor']) && $_POST['lookin']=='posts' ) {
		...
		$this->redirect( $C->SITE_URL.'search/tab:'.trim($_POST['lookin']).'/ptypes:link,image,video,file/s:'.urlencode(trim($tmp)) );
	}
	...
	elseif( $D->tab == 'posts' )
	{
		...
		$pgroup	= trim( preg_replace('/[^a-zа-я0-9\-\.\s]/iu', '', urldecode($this->param('pgroup'))) );
		$ptypes	= 'link,image,video,file';
		if( isset($this->params->ptypes) ) {
			$ptypes	= trim( preg_replace('/[^a-z\,]/', '', $this->params->ptypes) );
		}
		$ptypes	= explode(',', $ptypes);
		foreach($ptypes as $k=>$v) {
			if( $v!='link' && $v!='image' && $v!='file' && $v!='video' ) { unset($ptypes[$k]); }
		}
		...
		if( ! empty($D->search_string) ) {
			...
			if( !$D->error ) {
				...
				if( mb_strlen($D->search_string)>=3 && FALSE!==strpos($D->search_string,' ') ) {
					$tmp	= preg_replace('/[^а-яa-z0-9\s]/iu', '', $tmp);
					$tmp	= $db2->e($tmp);
