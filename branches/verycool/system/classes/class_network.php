	...
	class network
	{
		...
		public function get_recent_posttags($in_sql, $count=20, $force_refresh=FALSE)
		{
			...
			while($tmp = $this->db2->fetch_object()) {
				if( ! preg_match_all('/\#([а-яa-z0-9\-_]{1,50})/iu', stripslashes($tmp->message), $matches, PREG_PATTERN_ORDER) ) {
					continue;
				}
				$thisposttags	= array();
		...
		public function get_posts_api($id, $force_refresh=FALSE)
		{
			$id	= intval($id);
			static $loaded = array();
			$cachekey	= 'n:'.$this->id.',post_api:'.$id;
			if( isset($loaded[$cachekey]) && TRUE!=$force_refresh ) {
				return $loaded[$cachekey];
			}
			$data	= $this->cache->get($cachekey);
			...
			$r	= $this->db2->query('SELECT id, name FROM post_api WHERE id="'.$id.'" LIMIT 1', FALSE);
		...
		public function send_notification_post($to_user_id, $in_group_id, $lang_key, $lang_params, $if_exists_action='ignore')
		{
			...
			if( $to_user_id > 0 ) {
				...
				$this->db2->query('INSERT INTO post_userbox SET user_id="'.$to_user_id.'", post_id="'.intval($this->db2->insert_id()).'" ');
			}
			elseif( $in_group_id > 0 ) {
				...
				$this->db2->query('INSERT INTO posts SET group_id="'.$in_group_id.'", message="'.$data.'", date="'.time().'", ip_addr="'.ip2long($_SERVER['REMOTE_ADDR']).'" ', FALSE);
			}
		}
